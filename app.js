...
const Charts = {
  _base() {
    return {
      ...
      plugins:{
        ...
        tooltip:{
          backgroundColor:'rgba(1,2,5,0.98)', borderColor:'rgba(0,200,255,0.20)', borderWidth:1,
  ...
  bar(id, labels, datasets, opts={}) {
    ...
    const mkGrad=(hex)=>{
      ...
      g.addColorStop(0.5,h+'1a');
      g.addColorStop(1,h+'04');
      return g;
    };
    const mkBg=(d,i)=>{
    ...
    const mkBorder=(d,i)=>{
      if(d.colors) return d.colors.map(c=>c.slice(0,7)+'33');
  ...
  donut(id, labels, data, opts={}) {
    ...
    State.charts[id]=new Chart(canvas,{
      ...
      data:{ labels, datasets:[{
        data,
        backgroundColor:dPal.map(c=>c+'44'),
        borderColor:dPal,
        borderWidth:2.5,
        hoverBackgroundColor:dPal,
        hoverBorderColor:dPal.map(c=>c+'33'),
        hoverBorderWidth:3,
        hoverOffset:14
      }]},
      options:cfg, plugins
  ...
  polar(id, labels, data, opts={}) {
    ...
    State.charts[id]=new Chart(canvas,{
      type:'polarArea',
      data:{ labels, datasets:[{ data, backgroundColor:C.PALETTE.slice(0,data.length).map(c=>c+'33'), borderColor:C.PALETTE.slice(0,data.length), borderWidth:1.5 }]},
      options:cfg
    });
    canvas.addEventListener('click',()=>{ Sound.unlock(); Sound.chart(); },{passive:true});
    return State.charts[id];
  ...
  scatter(id, datasets, opts={}) {
    ...
    State.charts[id]=new Chart(canvas,{
      type:'scatter',
      data:{ datasets:datasets.map((d,i)=>({ borderColor:C.PALETTE[i%C.PALETTE.length], backgroundColor:C.PALETTE[i%C.PALETTE.length]+'22', pointRadius:6, pointHoverRadius:9, ...d }))},
      options:cfg
    });
    canvas.addEventListener('click',()=>{ Sound.unlock(); Sound.chart(); },{passive:true});
    return State.charts[id];
...
async function initCashPage() {
  ...
  Charts.bar('cash-bar',labels,[{label:'Cash (LKR)',data:vals,colors:C.PALETTE.slice(0,vals.length).map(c=>c+'44')}],{horizontal:true,currency:true});
...
async function initFinancePage() {
  ...
  if (!document.getElementById('fin-col-style')) {
    const fcs = document.createElement('style');
    fcs.id = 'fin-col-style';
    fcs.textContent = `
      #fin-tbl th[data-col="Loan Account No"], #fin-tbl td:nth-child(3) { width:120px; min-width:120px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; font-size:.63rem; }
      #fin-tbl th[data-col="Age Days"] { white-space:normal; word-break:break-word; width:55px; max-width:55px; text-align:center; font-size:.63rem; line-height:1.2; padding:4px; }
      #fin-tbl td:nth-child(7) { width:55px; max-width:55px; text-align:center; font-size:.63rem; padding-left:4px; padding-right:4px; }
    `;
    document.head.appendChild(fcs);
  }
...
async function initTenderPage() {
  ...
  function renderView(rows) {
    ...
    // ── Production Progress — custom visual bar design ──
    (()=>{
      const canvas = document.getElementById('tender-progress');
      if (!canvas) return;
      const container = canvas.closest('.chart-box');
      if (!container) return;
      canvas.style.display = 'none';

      // Remove previous custom progress if any
      container.querySelector('.custom-progress-list')?.remove();

      // Only ongoing tenders
      const displayRows = allRows.filter(r => r._tabStatus === 'On-Going').slice(0, 14);

      function getBarGradient(p) {
        if (p >= 90)  return 'linear-gradient(90deg,#1e1b4b,#312e81)';
        if (p >= 80)  return 'linear-gradient(90deg,#1c1917,#44403c)';
        if (p >= 65)  return 'linear-gradient(90deg,#1c0a00,#431407)';
        if (p >= 50)  return 'linear-gradient(90deg,#1c1200,#451a03)';
        if (p >= 35)  return 'linear-gradient(90deg,#052e16,#14532d)';
        if (p >= 20)  return 'linear-gradient(90deg,#022c22,#064e3b)';
        return         'linear-gradient(90deg,#0c1a2e,#0f2942)';
      }
      function getStatus(p) {
        if (p >= 90)  return {label:'NEARLY DONE', color:'#a78bfa'};
        if (p >= 80)  return {label:'AHEAD',        color:'#fb923c'};
        if (p >= 65)  return {label:'IN PROGRESS',  color:'#f97316'};
        if (p >= 35)  return {label:'IN PROGRESS',  color:'#fbbf24'};
        if (p >= 10)  return {label:'ON TRACK',     color:'#4ade80'};
        return               {label:'STARTING',     color:'#60a5fa'};
      }

      const wrap = document.createElement('div');
      wrap.className = 'custom-progress-list';
      wrap.style.cssText = 'display:flex;flex-direction:column;gap:10px;padding:10px 4px;height:100%;overflow-y:auto;box-sizing:border-box;';

      // Inject style once
      if (!document.getElementById('custom-prog-style')) {
        const s = document.createElement('style');
        s.id = 'custom-prog-style';
        s.textContent = `
          .cp-row { display:flex; align-items:center; gap:10px; min-height:32px; }
          .cp-label { font-size:.60rem; font-weight:700; color:rgba(255,255,255,.55); width:72px; min-width:72px; text-align:right; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; letter-spacing:.3px; }
          .cp-bar-wrap { flex:1; background:rgba(255,255,255,0.07); border-radius:20px; height:22px; position:relative; overflow:hidden; }
          .cp-bar-fill { height:100%; border-radius:20px; position:relative; transition:width 1.1s cubic-bezier(.34,1.15,.64,1); display:flex; align-items:center; justify-content:flex-end; }
          .cp-pct { font-size:.68rem; font-weight:800; color:#fff; padding-right:8px; text-shadow:0 1px 4px rgba(0,0,0,.5); white-space:nowrap; letter-spacing:.4px; }
          .cp-status { font-size:.56rem; font-weight:800; width:80px; min-width:80px; text-align:left; letter-spacing:.6px; }
          .cp-dot { width:7px; height:7px; border-radius:50%; display:inline-block; margin-right:4px; flex-shrink:0; box-shadow:0 0 5px currentColor; }
        `;
        document.head.appendChild(s);
      }

      displayRows.forEach(r => {
        const tot = r['Total Tender Qty'] || r['Tender Qty'] || 0;
        const cast = r['Total Qty Casted'] || 0;
        const p = tot ? Math.min(100, Math.round(cast / tot * 100)) : (r._tabStatus === 'Completed' ? 100 : 0);
        const grad = getBarGradient(p);
        const st = getStatus(p);
        const label = r['Site Name'] || r['Tender No'] || '—';

        const row = document.createElement('div');
        row.className = 'cp-row';
        row.innerHTML = `
          <div class="cp-label" title="${label}">${label}</div>
          <div class="cp-bar-wrap">
            <div class="cp-bar-fill" style="width:0%;background:${grad}">
              <span class="cp-pct">${p}%</span>
            </div>
          </div>
          <div class="cp-status" style="color:${st.color}">
            <span class="cp-dot" style="background:${st.color};color:${st.color}"></span>${st.label}
          </div>`;
        wrap.appendChild(row);

        // Animate bar after paint
        setTimeout(() => {
          row.querySelector('.cp-bar-fill').style.width = p + '%';
        }, 80);
      });
    ...
    if (gaugesEl) {
      if (!ongoingForGauges.length) {
        ...
        gaugesEl.innerHTML=ongoingForGauges.map((r,i)=>{
          ...
          const limitCol = isExpired ? '#f43f5e' : '#10b981';
          // Arc from left end up to needle position (partial arc)
          const needleEndX = (cx - rad) + 2 * rad * (1 - displayPct / 100); // not used directly
          // Partial filled arc: from left (180°) to needle angle
          const startX = cx - rad, startY = cy;
          const endX = needleTipX + (cx - cx); // tip projected onto arc
          // Use SVG arc: from (cx-rad, cy) sweeping right by displayPct%
          const sweepAngle = (displayPct / 100) * Math.PI;
          const arcEndX = cx + rad * Math.cos(Math.PI - sweepAngle);
          const arcEndY = cy - rad * Math.sin(Math.PI - sweepAngle);
          const filledArc = `M ${cx - rad} ${cy} A ${rad} ${rad} 0 0 1 ${arcEndX.toFixed(2)} ${arcEndY.toFixed(2)}`;
          const fullArcTrack = `M ${cx-rad} ${cy} A ${rad} ${rad} 0 0 1 ${cx+rad} ${cy}`;
          ...
          return `<div style="text-align:center;min-width:95px;${isExpired?'animation:expiredPulse 1.2s ease-in-out infinite;':''}" title="${r['Tender No']||''} · ${poleType} · Limit: ${limitDays}d">
            <svg viewBox="0 0 88 52" width="95" height="56" style="overflow:visible;display:block;margin:0 auto">
              ...
              <!-- Background track — dim, no color -->
              <path d="${fullArcTrack}" fill="none" stroke="rgba(255,255,255,0.08)" stroke-width="7" stroke-linecap="round"/>
              <!-- Filled arc left → needle — red or green only -->
              <path d="${filledArc}" fill="none" stroke="${limitCol}" stroke-width="7" stroke-linecap="round" filter="url(#glow-${i})"/>
              <!-- Needle stem -->
              <path d="${needleStem}" stroke="${limitCol}" stroke-width="1.5" stroke-linecap="round" opacity="0.9"/>
              <!-- Needle arrowhead -->
              <path d="${arrowHead}" fill="${limitCol}" style="filter:drop-shadow(0 0 3px ${limitCol})"/>
              <!-- Pivot -->
              <circle cx="${cx}" cy="${cy}" r="2.5" fill="${limitCol}" stroke="rgba(0,0,0,.7)" stroke-width="1"/>
              <!-- Value -->
              <text x="${cx}" y="${cy-7}" text-anchor="middle" font-size="8.5" font-weight="800" fill="${limitCol}" font-family="monospace">+${Math.round(ageDays)}d</text>
            </svg>
            <div style="font-size:.55rem;font-weight:700;color:${limitCol};margin-top:.04rem;letter-spacing:.4px">△ +${Math.round(ageDays)}d</div>
    ...
    function buildDelayMeter(delayVal, rowIdx) {
      ...
      const limitCol = isDelayed ? '#f43f5e' : '#10b981';
      const sweepAngle = (fillPct / 100) * Math.PI;
      const arcEndX = cx + rad * Math.cos(Math.PI - sweepAngle);
      const arcEndY = cy - rad * Math.sin(Math.PI - sweepAngle);
      const filledArc = `M ${cx-rad} ${cy} A ${rad} ${rad} 0 0 1 ${arcEndX.toFixed(2)} ${arcEndY.toFixed(2)}`;
      const fullArcTrack = `M ${cx-rad} ${cy} A ${rad} ${rad} 0 0 1 ${cx+rad} ${cy}`;

      return `<div class="inline-meter-wrap${isDelayed?' m-delayed':''}" style="text-align:center;min-width:60px;">
        <svg viewBox="0 0 88 52" width="60" height="36" style="overflow:visible;display:block;margin:0 auto">
          ...
          <path d="${fullArcTrack}" fill="none" stroke="rgba(255,255,255,0.08)" stroke-width="7" stroke-linecap="round"/>
          <!-- Filled arc left → needle — red or green only -->
          <path d="${filledArc}" fill="none" stroke="${limitCol}" stroke-width="7" stroke-linecap="round" filter="url(#${filtId})"/>
          <!-- Needle stem -->
          <path d="${needleStem}" stroke="${limitCol}" stroke-width="1.5" stroke-linecap="round" opacity="0.9"/>
          <!-- Needle arrowhead -->
          <path d="${arrowHead}" fill="${limitCol}" style="filter:drop-shadow(0 0 3px ${limitCol})"/>
          <circle cx="${cx}" cy="${cy}" r="2.5" fill="${limitCol}" stroke="rgba(0,0,0,.7)" stroke-width="1"/>
          <text x="${cx}" y="${cy-7}" text-anchor="middle" font-size="8.5" font-weight="800" fill="${limitCol}" font-family="monospace">${valTxt}</text>
        </svg>
...
async function initSleepersPage() {
  ...
  function renderView(rows) {
    ...
    Charts.bar('slp-combined',slpLabels,[
      ...
      {label:'Balance to Cast (pcs)',data:slpBalance, colors:slpCompleted.map(c=>c?C.green+'18':C.amber+'33')}
    ...
    if (ogRows.length===1) {
      ...
      if (dynSites.length) {
        ...
        Charts.bar('slp-stacked',dynSites,[
          {label:'Casted',           data:siteCasted,  colors:dynSites.map(()=>C.green+'44')},
          {label:'Balance to Cast',  data:siteBalance, colors:dynSites.map(()=>C.amber+'44')}
...
async function initVehiclePage() {
  ...
  function renderView(rows) {
    ...
    Charts.bar('veh-km',Object.keys(byVeh),[{label:'Distance (km)',data:Object.values(byVeh),colors:Object.keys(byVeh).map((_,i)=>C.PALETTE[i%C.PALETTE.length]+'44')}],{unit:' km'});

    const byVehFuel={};
    rows.forEach(r=>{ const v=r['Vehicle Number']||'?'; byVehFuel[v]=(byVehFuel[v]||0)+(r['Total Fuel Consumed (L)']||0); });
    Charts.bar('veh-fuel',Object.keys(byVehFuel),[{label:'Fuel (L)',data:Object.values(byVehFuel),colors:Object.keys(byVehFuel).map((_,i)=>C.PALETTE[(i+3)%C.PALETTE.length]+'44')}],{unit:' L'});
    ...
    Charts.scatter('veh-scatter',[{label:'Vehicles',data:scatterPts,backgroundColor:C.primary+'33',borderColor:C.primary}],{xLabel:'Distance (km)',yLabel:'Mileage (km/L)'});
